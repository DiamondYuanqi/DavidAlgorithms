...
def select_sorted_population(fitness, population, length):
    ...
    sorted_key = sorted(sort_dict.keys(), key=operator.itemgetter(0), reverse=True) #排序，反转  value 为 key=operator.itemgetter(0)

    sorted_index = [sort_dict[i] for i in sorted_key]  #索引
...
def selection(fitness, num):
    def select_one(fitness, fitness_sum):
        ...
        while True:
            if random.random() < fitness[i] / fitness_sum: #选择算子
                return i
            else:
                i = (i + 1) % size #轮盘
...
def get_distance(sequence):
    ...
    cost = 0
    for i in range(len(sequence)): #城市序列
        cost += distmat[sequence[i - 1]][sequence[i]] #累计计算距离
    return cost
...
def get_distmat(M):
    length = M.shape[0]
    distmat = np.zeros((length, length)) #距离矩阵

    for i in range(length):
        for j in range(i + 1, length):                                  #二范数 求欧式距离
            distmat[i][j] = distmat[j][i] = np.linalg.norm(M[i] - M[j]) #linalg=linear（线性）+algebra（代数），norm则表示范数。
    return distmat  #返回距离矩阵

def main():
    global p_mutation, max_generation #全局变量 迭代最大数
    generation = 1

    population_cur = init_population() #初始化种群
